  ...
  <div>
    <section class="col-xs-12" v-if="false">
      <section class="col-xs-9" style="padding-left: 0px; margin-left: 0px">
        <h3 style="padding-left: 0px; margin-left: 0px">Cadastro {{titulo_cadastro}}</h3>
        ...
        <div class="box-body">
          ...
          <div class="col-xs-8">
            <div class="form-group" v-if="post_type=='iniciativa' ">
              ...
              <input
                ...
                name="f_nome"
                id="f_detalhe"
                v-model="form.dados.objetivo"
                type="text"
                placeholder="Objetivo da iniciativa"
              />
              <textarea
                class="form-control"
                v-model="form.dados.objetivo"
                name="f_dados_objetivo"
                id="f_dados_objetivo"
                style="height: 200px"
                v-if="false"
              ></textarea>
          ...
          <div class="col-xs-4">
            <div class="form-group">
              ...
              <select class="form-control" name="f_status" id="f_status" v-model="form.status">
                <option
                  :value="item.id"
          ...
          <div class="col-xs-6">
            <div class="form-group">
              ...
              <input
                ...
                name="f_categoria"
                id="f_categoria"
                v-model="form.dados.categoria"
                type="text"
                placeholder="Categoria (separe por vírgula) "
              />
          ...
          <div class="col-xs-6">
            <div class="form-group" v-if="post_type=='iniciativa' ">
              ...
              <checkboxlist
                v-model="form.dados.objetivo_ods"
                :lista="lista_ods"
                name="f_dados_objetivo_ods"
          ...
          <div class="col-xs-4">
            <div class="form-group">
              ...
              >
                <option
                  :value="item.id"
                  v-for="(item, index) in list_categorias"
                  :key="index"
                >{{item.nome}}</option>
              </select>
            </div>
          ...
          <div class="col-xs-8">
            <div class="form-group">
              ...
              <input
                ...
                name="f_recursos"
                id="f_recursos"
                v-model="form.dados.recursos"
                type="text"
                placeholder="Recursos"
              />
              {{exibe_error('recursos')}}
            </div>
          </div>
          ...
          <div class="col-xs-12" v-if="form.id != null && parseInt(form.id) > 0 && show_contato">
            <div class="form-group">
              <recurso_cadastros_list_cadtable
                :id_recurso="form.id"
                titulo="de Forma de Contato"
                tipo="contato"
                titulo_coluna_inicio="Link Rede Social ou Contato"
                :recurso_id="form.id"
                :onSaveData="saveDataCadastros"
              ></recurso_cadastros_list_cadtable>
            </div>
          </div>
          ...
          <div class="form-group" v-if="false">
            ...
            <input
              ...
              name="f_tipo_fluxo"
              id="f_tipo_fluxo"
              v-model="form.tipo_fluxo"
              type="text"
              placeholder="tipo_fluxo"
            />
            {{exibe_error('tipo_fluxo')}}
          </div>
        </div>
      </div>
    </section>
...
Vue.component('recurso_list', RecursoList );
      ...
      <div class="form-group">
        <div class="btn-group pull-right">
          ...
          <button
            ...
            v-bind:disabled="disableButton"
            v-if="form.id!=null && parseInt(form.id) > 0"
            v-on:click="excluir"
          >Excluir</button>
        </div>
      </div>
    ...
    >
      <div class="box">
        <div class="box-header with-border">Potencialidades e Possibilidades</div>
        <div class="box-body">
          <recurso_associacao :id_recurso_pai="form.id"></recurso_associacao>
        </div>
      </div>
    </section>
    <section
    ...
    >
      <div class="box">
        <div class="box-header with-border">Upload de arquivos</div>
        <div class="box-body">
          <upload-folder :id_registro="form.id" id_tabela="recurso"></upload-folder>
        </div>
      </div>
    </section>
  </div>
...
import serviceEndereco from "../../services/EnderecoService";
...
export default {
  ...
  data: function() {
    return {
      ...
      form_endereco: serviceEndereco.getModel(),
  ...
  methods: {
    ...
    salvar(tipo) {
      ...
      if (this.form.id != null && this.form.id.toString() != "") {
        method = "POST";
        url = "recursos_edit/" + this.form.id;
      }
    ...
    excluir() {
      ...
      obj_alert.confirm(
        "Atenção",
        "Deseja realmente excluir esta " + this.titulo_cadastro + "?",
        "question",
        function(question) {
          if (question.value) {
