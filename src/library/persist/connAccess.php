...
class connAccess{
	   ...
	   public static function formatValue( $value ){
	    ...
	    public static function formatValue2( $value ){
	   ...
	   public static function Insert(IDbPersist $oConn, $arr, $table, $pk, $auto_increment = true){
			 ...
			 foreach( $arr as $key=>$value ){
			        ...
			        if ( $key == "cep"){
			             $values .= connAccess::formatValue2($arr[$key]);
		...
		//if ( $table == "inspecao_item_deleted" )
		  //  die( $sql);
		return connAccess::executeScalar($oConn, "select max( ".$pk. ") as maxid from ". $table );
	   ...
	   public static function Update(IDbPersist $oConn, $arr, $table, $pk){
			 ...
			 foreach( $arr as $key=>$value ){
				 ...
				 $fields .= $oConn->formatField($key) . " = ";
				   ...
				   if ( $key == "cep"){
					         $fields .=  connAccess::formatValue2($arr[$key]);

				   }else{
					         $fields .=  connAccess::formatValue($arr[$key]);
